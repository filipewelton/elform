defmodule Elform.Utilities.ActionFallback do
  defmacro __using__(opts) do
    ...
    if name == nil or type == nil do
      quote do
        def call(_, _) do
          {:error, "The action fallback must receive name and type arguments"}
        end
      end
    else
      quote do
        ...
        cond do
          unquote(type) == :string ->
            def call(value, _arg) when not is_bitstring(value) do
              [{unquote(name), "the value must be a string"}]
            end

          unquote(type) == :number ->
            def call(value, _arg) when not is_number(value) do
              [{unquote(name), "the value must be a number"}]
            end
          ...
          unquote(type) == :boolean ->
            def call(value, _arg) when not is_boolean(value) do
              [{unquote(name), "the value must be a boolean"}]
            end

          unquote(type) == :list ->
            def call(value, _arg) when not is_list(value) do
              [{unquote(name), "the value must be a list"}]
            end

          unquote(type) == :map ->
            def call(value, _arg) when not is_map(value) do
              [{unquote(name), "the value must be a map"}]
            end
